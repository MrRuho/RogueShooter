...
public class ShootAction : BaseAction
{
    ...
    private void Shoot()
    {
        ...
        switch (result.tier)
        {
            ...
            // Vähentää vain suojaa.
            case ShotTier.Miss:
                ApplyHit(result.damage, result.bypassCover, result.coverOnly, targetUnit);
                Debug.Log("Miss! Cover Damage only: " + result.damage);
                return;

            // Vähentää suojaa. Jos ei suojaa niin osuu. "Hipaisu"
            case ShotTier.Graze:
            ...
            // Täysi osuma. Tekee vahinkoa kokonaisuudessaan.
            case ShotTier.Hit:
                if (GetCoverType(targetUnit) == CoverService.CoverType.None)
                {
                    MakeDamage(result.damage + weapon.NoCoverDamageBonus, targetUnit);
